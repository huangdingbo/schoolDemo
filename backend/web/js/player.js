...
class Player {
    constructor(keyword) { //类的构造函数
        keyword = keyword == undefined ? 0 : keyword

        //如果类已经有实例了，就返回这个实例
        // if (Player.instance) return Player.instance;
        //如果没有实例化，就去构造一个实例
        return this.getInstance(keyword);
    }

    //构建实例
    getInstance(keyword) {
        let instance = new PlayerCreator(keyword);
...
class Musics {
    ...
    constructor(keyword) {
        ...
        $.ajax({
            type: 'GET',
            url: "http://huangdingbo.work/school/backend/web/music/get-songs?keyword="+keyword,
...
class PlayerCreator {
    constructor(keyword) {
        ...
        //工具
        this.util = new Util();
        this.musics = new Musics(keyword); //歌曲信息
